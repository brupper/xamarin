...
{
    public IPlatformInformationService PlatformInformation => ServiceProvider.GetService<IPlatformInformationService>();
    ...
    /// <summary>
    /// <para> PermissionException: Permission request must be invoked on main thread. </para>
    ...
    private async Task InternalnitializeGeolocationAsync()
    {
        try
        {
            // PermissionException: Permission request must be invoked on main thread.
